...
const templatePath = path.join(packageRoot, "interactive-demo.template.html");
...
async function readDemoAssets(demo: InteractiveDemo): Promise<DemoAssets> {
  ...
  const jsHash = js.file.match(new RegExp(`^${demo.name}-([a-f0-9]+)\\.js$`))?.[1];
...
async function writeSharedShell(demos: readonly InteractiveDemo[]) {
  ...
  const loader = await fs.readFile(path.join(shellRoot, "interactive-demo.js"), "utf8");
  ...
  await fs.writeFile(path.join(publicDemosRoot, "interactive.html"), versionedShell);
  await Promise.all(
    demos.map((demo) =>
      fs.rm(path.join(publicDemosRoot, demo.name, "index.html"), { force: true }),
    ),
  );
  await fs.writeFile(path.join(publicDemosRoot, "interactive-demo.js"), loader);
  await fs.copyFile(
...
async function withGeneratedHtml(demos: readonly InteractiveDemo[], action: () => void) {
  try {
