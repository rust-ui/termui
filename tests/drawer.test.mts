...
test("Open Trigger opens profile Drawer; inside clicks stay open and outside clicks close", async () => {
  ...
  try {
    await page.goto(`${session.baseUrl}/demos/interactive.html?demo=drawer-interactive`);
    ...
    const initialLayout = await page
      ...
      .evaluateAll((rows) => {
        const locate = (text: string) => {
          const row = rows.findIndex((line) => line.textContent?.includes(text));
    ...
    const triggerCenter = initialLayout.trigger.column + initialLayout.trigger.length / 2;
